...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("up_move_optl") and is_on_floor():
		velocity.y = JUMP_VELOCITY
	
	# Obtener la dirección de movimiento horizontal (izquierda o derecha)
	var direction := Input.get_axis("left_move_optl", "right_move_optl")
...
func _on_area_2d_body_entered(area) -> void:
	if area.is_in_group("Player_1"):
		print(self.name, " chocó con " ,"Player_1")
